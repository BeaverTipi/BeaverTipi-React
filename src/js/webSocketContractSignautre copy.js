...
export const createWebSocketHandlers = ({
  ...
  const initWebSocket = () => {
    ...
    ws.onopen = () => {
      ...
      const payload = {
        signers: { [myRole]: signerInfo }
      };
      isAwaitingInitRef.current = true;
      sendMsg(MSG.INIT_REQUEST, payload);
  ...
  const realtimeWebSocket = () => {
    ...
    ws.onopen = () => console.debug("[REALTIME_WS] 연결됨");
    ws.onmessage = event => {
      ...
      switch (type) {
        ...
        case MSG.INIT_RESPONSE:
          ...
          if (isAwaitingInitRef.current) {
            ...
            const dynamicType = `SET_SIGNERS_${role}`;
            dispatch({ type: MSG[dynamicType], payload: payload.signers?.[role] });
