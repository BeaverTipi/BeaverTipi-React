...
export default function ContractSpecialTerms() {
  return (
    <ComponentCard title="특약사항" desc="임대인과 임차인 간의 특약 내용을 명시해주세요.">
      <div className="space-y-4 text-sm">
        {/* 전입신고 및 확정일자 */}
        <div className="grid grid-cols-4 gap-4 items-end">
          <label className="col-span-1 font-semibold">전입신고 마감일</label>
          <Input name="moveInDeadline" placeholder="YYYY-MM-DD" className="col-span-2" />
        </div>
        ...
        <div>
          ...
          <Checkbox name="agreeMediation" label="조정 신청에 동의합니다 (그릇된 분쟁 방지 목적)" />
        </div>

        {/* 주택 및 그 외 자산에 관한 구체적 계획 */}
        <div className="space-y-2">
          <label className="font-semibold">추후 자산 계획</label>
          <Textarea name="assetPlan" placeholder="예: 매물 2호점 구축 계획 등..." rows={2} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex gap-4 items-center">
            <Radio name="detailedAddrFee" value="agree" label="동의" />
            <Radio name="detailedAddrFee" value="disagree" label="미동의" />
          </div>
        </div>
      </div>
    </ComponentCard>
